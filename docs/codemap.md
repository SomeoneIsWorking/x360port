...
| XAM controller-state guest service | `include/x360port/xam_input.hpp`, `src/xam_input.cpp` | Bind the platform `xam.xex` ordinal 401, serialize the 16-byte big-endian controller state into checked guest memory, and return connection status; the consuming title supplies a coherent controller snapshot and user policy. |

...
| Xenia export/import attachment and callback lifetime | `src/runtime_imports.{hpp,cpp}`, `include/x360port/validation.hpp` | Keep manifest-owned names/tables alive through resolver teardown; bind only validated title-neutral imports and expose register arguments, return propagation, and bounded guest-memory access through `GuestImportContext`. |
